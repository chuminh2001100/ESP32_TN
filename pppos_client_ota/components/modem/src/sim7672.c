 ...
 */
static const char *DCE_TAG = "sim7672";
 ...
 * @brief Handle response from AT+CBC
 */
static esp_err_t sim7600_handle_location(modem_dce_t *dce, const char *line)
{
    esp_err_t err = ESP_FAIL;
    esp_modem_dce_t *esp_modem_dce = __containerof(dce, esp_modem_dce_t, parent);
    if (strstr(line, MODEM_RESULT_CODE_SUCCESS)) {
        ...
        err = esp_modem_process_command_done(dce, MODEM_STATE_FAIL);
    } else if (!strncmp(line, "+CLBS", strlen("+CLBS"))) {
        uint32_t location[4];
        int result;
        sscanf(line, "+CLBS: %d,%d.%d,%d.%d", &result, &location[0],
                                            &location[1], &location[2], &location[3]);
        char *location_s = esp_modem_dce->priv_resource;
        location_s[0] = '\0';
        if(result == 0){
            sprintf(location_s, "%d.%d,%d.%d", location[0],location[1], location[2], location[3]);
        }
        ESP_LOGI(DCE_TAG, "Location: %s", line);
    }
    return err;
}
/**
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t sim7600_get_location(modem_dce_t *dce, char *location)
{
    modem_dte_t *dte = dce->dte;
    esp_modem_dce_t *esp_modem_dce = __containerof(dce, esp_modem_dce_t, parent);
    esp_modem_dce->priv_resource = location;
    dce->handle_line = sim7600_handle_location;
    DCE_CHECK(dte->send_cmd(dte, "AT+CLBS=4\r", MODEM_COMMAND_TIMEOUT_MODE_CHANGE) == ESP_OK, "send command failed", err);
    DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "get location error", err);
    ESP_LOGD(DCE_TAG, "get location ok");
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
// esp_err_t esp_modem_dce_handle_gps(modem_dce_t *dce, const char *line)
// {
//     esp_err_t err = ESP_FAIL;
//     if (strstr(line, MODEM_RESULT_CODE_SUCCESS)) {
//         err = esp_modem_process_command_done(dce, MODEM_STATE_SUCCESS);
...
// esp_err_t esp_modem_dce_turn_on_gps(modem_dce_t *dce)
// {
//     modem_dte_t *dte = dce->dte;
//     dce->handle_line = esp_modem_dce_handle_response_default;
//     DCE_CHECK(dte->send_cmd(dte, "AT+CGPS=1\r", MODEM_COMMAND_TIMEOUT_DEFAULT) == ESP_OK, "send command failed", err);
//     DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "turn on GPS failed", err);
//     ESP_LOGD(DCE_TAG, "turn on GPS ok");
//     return ESP_OK;
// err:
//     return ESP_FAIL;
// }

// esp_err_t esp_modem_dce_read_gps(modem_dce_t *dce)
// {
//     modem_dte_t *dte = dce->dte;
//     dce->handle_line = esp_modem_dce_handle_response_default;
//     DCE_CHECK(dte->send_cmd(dte, "AT+CGPSINFO\r", MODEM_COMMAND_TIMEOUT_DEFAULT) == ESP_OK, "send command failed", err);
//     DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "read GPS failed", err);
//     ESP_LOGD(DCE_TAG, "read GPS ok");
//     return ESP_OK;
// err:
//     return ESP_FAIL;
// }
...
modem_dce_t *sim7600_init(modem_dte_t *dte)
{
    ...
    if(dce != NULL)
    {
        ...
        dte->dce->get_location = sim7600_get_location;
 ...
 *
 */
bool sim7600_reinit(modem_dce_t *dce)
{
    return bg96_reinit(dce);
}
