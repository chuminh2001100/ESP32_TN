...
bool bg96_reinit(modem_dce_t *dce)
{
/* Sync between DTE and DCE */
    DCE_CHECK(esp_modem_dce_sync(dce) == ESP_OK, "sync failed", err_io);
    /* Close echo */
    DCE_CHECK(esp_modem_dce_echo(dce, false) == ESP_OK, "close echo mode failed", err_io);
    /* Get Module name */
    DCE_CHECK(esp_modem_dce_get_module_name(dce) == ESP_OK, "get module name failed", err_io);
    /* Get IMEI number */
    DCE_CHECK(esp_modem_dce_get_imei_number(dce) == ESP_OK, "get imei failed", err_io);
    /* Get IMSI number */
    DCE_CHECK(esp_modem_dce_get_imsi_number(dce) == ESP_OK, "get imsi failed", err_io);
    /* Get operator name */
    DCE_CHECK(esp_modem_dce_get_operator_name(dce) == ESP_OK, "get operator name failed", err_io);
